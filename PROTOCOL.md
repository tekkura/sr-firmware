...
| `0x06` | `GET_VERSION`     | Requests firmware version           | `VERSION`          |
...
### `VERSION` (3 bytes)
The `GET_VERSION` response payload contains three bytes in order:
`version_major`, `version_minor`, and `version_patch`. These are numeric
SemVer components (for example, `0x01 0x01 0x00` represents version `1.1.0`).
