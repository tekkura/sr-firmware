 ...
 * Protocol: TinyFrame (SOF=0x01, Checksum=CRC16-CCITT)
 */
...
const int EXPECTED_RESPONSE_SIZE = 38; // 9 bytes TF overhead + 29 bytes RP2040_STATE

// --- CRC16-CCITT Implementation (Matches TinyFrame's Reflected CRC16) ---
...
std::vector<uint8_t> wrap_tinyframe(uint8_t type, const std::vector<uint8_t>& data) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::vector<uint8_t> tx_packet = wrap_tinyframe(CMD_SET_MOTOR, payload);
            ...
            while(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - t_start).count() < 200) {
                if (read(serial.fd, &c, 1) > 0) {
                    ...
                    if (rx.size() == EXPECTED_RESPONSE_SIZE) {
                        ...
                        // Basic validation: SOF and Type
                        // Response ID is same as Request ID, which is rx[1]. Type is rx[4].
                        if (rx[0] == TF_SOF && rx[4] == CMD_SET_MOTOR) {
                            status = 1;
                        } else if (debug) {
                            std::cout << "\n[DEBUG] Unexpected packet contents. SOF: 0x" << std::hex << (int)rx[0] << " Type: 0x" << (int)rx[4] << std::dec << std::endl;
                        }
                        break;
                    }
                }
            ...
            if (status == 1) {
                ...
                if (debug) {
                    if (rx.empty()) {
                        ...
                        std::cout << "\n[DEBUG] Partial/Wrong RX size: " << rx.size() << " bytes: ";
