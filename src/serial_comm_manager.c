...
void serial_comm_manager_init(RP2040_STATE* rp2040_state){
    outgoing_packet_to_android.state = *rp2040_state;
...
static inline void record_command_received(void) {
...
TF_Result listener_set_motor_level(TinyFrame *tf, TF_Msg *msg) {
    ...
    if (msg->len >= 2) {
        ...
        outgoing_packet_to_android.state.MotorsState.ControlValues.left = msg->data[0];
        outgoing_packet_to_android.state.MotorsState.ControlValues.right = msg->data[1];

        // Apply to hardware
        set_motor_levels(&outgoing_packet_to_android.state);

        // Refresh full state for response
        get_state(&outgoing_packet_to_android.state);
...
#endif
        response.data = (uint8_t*)&outgoing_packet_to_android;
        response.len = sizeof(outgoing_packet_to_android);
