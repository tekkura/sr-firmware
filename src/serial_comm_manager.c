...
static OutgoingPacketToAndroid outgoingPacketToAndroid;
...
void serial_comm_manager_init(RP2040_STATE* rp2040_state){
    outgoingPacketToAndroid.state = *rp2040_state;
}
...
void get_block() {
    ...
    outgoingPacketToAndroid.telemetry.t4_timestamp_us = time_us_64();
#endif
    // Read Length (2 bytes)
    int l1 = read_byte();
    ...
    switch (cmd) {
        ...
        case SET_MOTOR_LEVEL: {
            if (payload_len == 2) {
                ...
                memset(&outgoingPacketToAndroid.state, 0, sizeof(RP2040_STATE));

                outgoingPacketToAndroid.state.MotorsState.ControlValues.left = left;
                outgoingPacketToAndroid.state.MotorsState.ControlValues.right = right;
                set_motor_levels(&outgoingPacketToAndroid.state);

                get_state(&outgoingPacketToAndroid.state);
#ifdef ENABLE_LATENCY_BENCHMARK
                outgoingPacketToAndroid.telemetry.t5_timestamp_us = time_us_64();
#endif
                send_framed_packet(SET_MOTOR_LEVEL, (uint8_t*)&outgoingPacketToAndroid, sizeof(outgoingPacketToAndroid));
            } else {
                send_framed_packet(NACK, NULL, 0);
            }
            break;
        ...
        case GET_STATE: {
            ...
            get_state(&outgoingPacketToAndroid.state);
            send_framed_packet(GET_STATE, (uint8_t*)&outgoingPacketToAndroid, sizeof(outgoingPacketToAndroid));
            break;
        }
        case GET_VERSION: {
            VERSION version;
